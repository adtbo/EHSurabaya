    <header id="header" class="navbar-fixed-top navbar-inverse video-menu" role="" style="background-color:#000">
        <div class="container">
            <!-- <div class="row"> -->
                <div class="navbar-header ">
                        <a class="navbar-brand" href="#">
                            <img src="assets/videopage/images/logo.png" alt="" class="img-responsive">
                        </a>
                 </div><!--Navbar header End-->
                    <nav class="collapse navbar-collapse" id="bs-example-navbar-collapse-1" role="">
                        <ul class="nav navbar-nav navbar-right ">
                            <li><a href="<?php echo site_url('/'); ?>#home"> Home </a></li>
                            <li><a href="<?php echo site_url('/'); ?>#service"> Tentang Kami </a> </li>
                            <li><a href="<?php echo site_url('/'); ?>#team"> Aksi </a></li>
                            <li><a href="<?php echo site_url('/'); ?>#portfolio">Galeri </a> </li>
                            <li><a href="<?php echo site_url('/'); ?>#footer"> Kontak </a> </li>
                        </ul>
                    </nav>
        </div><!-- /.container-fluid -->
    </header>
    



